...
class Utils {
  ...
  static resolveInelasticCollision(obj, otherObj) {
    otherObj.velocity = obj.velocity = {
      x:
        (obj.mass * obj.velocity.x + otherObj.mass * otherObj.velocity.x) /
        (obj.mass + otherObj.mass),
      y:
        (obj.mass * obj.velocity.y + otherObj.mass * otherObj.velocity.y) /
        (obj.mass + otherObj.mass)
    }
  }
      ...
      if (obj.collision == 'elastic') {
        ...
        if (xVelocityDiff * xDist + yVelocityDiff * yDist >= 0) {
          ...
          const angle = -Math.atan2(
            otherObject.y - object.y,
            otherObject.x - object.x
          )
...
class Object {
  ...
  isTouching(obj) {
    if (obj.collider.shape == 'ellipse' && this.collider.shape == 'ellipse') {
      ...
      obj.collider.shape == 'ellipse' &&
      this.collider.shape == 'rect'
    ) {
    } else if (
      obj.collider.shape == 'rect' &&
      this.collider.shape == 'ellipse'
    ) {
    }
  }
...
let ellipticalShape = new Ellipse(500, 500)
/*
animate(() => {
  // Code for animation
})
